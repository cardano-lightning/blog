...
Protocols (in web, finance, communication etc.) to be useful require a whole ecosystem around them — libraries in different languages, great docs/specs, good user facing apps and users themselves. All that requires some maturity of the protocol and a lot of work on top of that.
...
Because the commitments are made by the customer only, the final version of the L2 story is just a "pile of cheques" that the recipient (like intermediary node - Payment Gateway) has to present to the cashier-blockchain smart contract to **instantly** get the money. Because the unidirectional commitment flow does not enforce an extra delay it facilitates nicely more dynamic liquidity management and batching.

Additionally as long as the other end accepted the payment, the customer does not really care about the state of the channel on the blockchain - nothing really beside he committed to pay can be withdrawn from the channel. The communication with L1 can be minimized to the individual events like opening, topping up or final withdrawal of the remaining balance.
...
There is of course a downside to this simplicity - the channel is unidirectional, so the customer cannot receive payments or receive **easily** refunds in a trust less way.

> Shortly mention here that unidirectionality means that the channel is just drained and there is no possible liquidity re-balancing.

> Sholud we mention a simple flow here with a bit of trust or a more complex trust less flow with squashing?
...
* When Bobs issues a off-chain cheque for 7 ADA to Alice, then in some sense those two cheques partially cancel each other and the on-chain 20 ADA is now sufficient to cover some future payments in both directions. Alice has 7 ADA and Bob has 13 ADA to spend.

This makes them especially useful between routing nodes, where traffic can naturally balance over time. They also enable fully trust less refunds and cancellations.
